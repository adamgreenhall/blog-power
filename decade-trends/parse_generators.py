...
def _get_file(year=2018):
    url_by_year = {
        2019: "https://www.eia.gov/electricity/data/eia860/xls/eia8602019.zip",
        2018: "https://www.eia.gov/electricity/data/eia860/archive/xls/eia8602018.zip",
...
def parse_generators(year=2018):
    ...
    df = df.assign(generator_uuid=df.plant_code.astype("str") + "_" + df.generator_id)[
        columns
    ].set_index("generator_uuid")
    assert df.index.is_unique
    df["planned_retirement_year"] = df.planned_retirement_year.replace(
        " ", np.nan
    ).astype(float)
    df["energy_source"] = df.energy_source_1.replace(energy_source_mapping)
    return df[
        (df.status == "OP") & df.sector.isin(["Electric Utility", "IPP Non-CHP", "IPP"])
    ].copy()


def parse_proposed(year=2018):
    df_proposed = pd.read_excel(_get_file(year), sheet_name="Proposed", skiprows=1)
    df_proposed = df_proposed.rename(
        columns={
            c: c.lower()
            .replace(" ", "_")
            .replace("?", "")
    ...
    df_proposed = df_proposed[
        ...
        & df_proposed.status.isin(
            ["TS", "V", "U", "T"]
        )  # must have regulatory approval or better
    ].copy()
    df_proposed["energy_source"] = df_proposed.energy_source_1.replace(
        energy_source_mapping
    )
    df_proposed.effective_year = df_proposed.effective_year.replace(" ", np.nan).astype(
        float
    )
    return df_proposed
...
def parse_retired(year=2018):
    ...
    df["energy_source"] = df.energy_source_1.replace(energy_source_mapping)
    df = df[
        df.sector.isin(["IPP Non-CHP", "Electric Utility"]) & (df.status == "RE")
    ].copy()[
        [
            "utility_name",
            "plant_name",
